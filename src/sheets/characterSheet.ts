...
export function buildCharacterSheet(): SpreadsheetState {
    ...
    // ─── HEADER ROW ───
    cells.push({ row: 0, col: 0, value: 'LEVEL', bold: true, fontSize: 10, bg: C.hdrBg, color: C.hdrText, align: 'center' });
    cells.push({ row: 0, col: 1, value: '1', locked: false, bold: true, fontSize: 18, bg: C.hdrBg, color: C.hdrAccent, align: 'center' });
    cells.push({ row: 0, col: 2, value: 'CLASS', bold: true, fontSize: 10, bg: C.hdrBg, color: C.hdrText, align: 'right' });
    cells.push({ row: 0, col: 3, value: 'Cleric', locked: false, bold: true, fontSize: 22, bg: C.hdrBg, color: C.hdrText, align: 'left' });
    for (let c = 4; c < NUM_COLS; c++) cells.push({ row: 0, col: c, value: '', bg: C.hdrBg });

    // Subtitle
    for (let c = 0; c < 3; c++) cells.push({ row: 1, col: c, value: '', bg: C.subBg });
    cells.push({ row: 1, col: 3, value: 'A Miraculous Priest of Divine Power', italic: true, fontSize: 10, bg: C.subBg, color: C.subText });
    ...
    for (const ab of abilities) {
        ...
        // Ability name header
        cells.push({ row: r, col: 0, value: ab.name, bold: true, fontSize: 10, bg: C.secBg, color: C.label, align: 'center' });
        cells.push({ row: r, col: 1, value: '', bg: C.secBg });

        // Score (editable) — row r+1, col B
        const scoreRef = cellKey(r + 1, 1);
        cells.push({ row: r + 1, col: 1, value: String(ab.score), locked: false, bold: true, fontSize: 20, align: 'center', bg: C.secBg, color: C.editable });

        // Modifier formula — row r+1, col A
        const modRef = cellKey(r + 1, 0);
        modRefs[ab.abbr] = modRef;
        cells.push({ row: r + 1, col: 0, value: `=ROUND((${scoreRef}-10)/2)`, bold: true, fontSize: 22, align: 'center', bg: C.secBg, color: C.value });

        // Save label — row r+2, col A
        cells.push({ row: r + 2, col: 0, value: `${ab.abbr} Save`, fontSize: 10, bg: C.secBg, color: C.label });

        // Save formula — row r+2, col B
        const profRef = 'F5';
        if (saveProficiencies[ab.abbr]) {
            cells.push({ row: r + 2, col: 1, value: `=${modRef}+${profRef}`, fontSize: 12, align: 'center', bg: C.secBg, color: C.acGreen });
        } else {
            cells.push({ row: r + 2, col: 1, value: `=${modRef}`, fontSize: 12, align: 'center', bg: C.secBg, color: C.label });
        }
        ...
        for (const skill of skills) {
            ...
            cells.push({ row: skillRow, col: 3, value: `${prefix} ${skill.name}`, fontSize: 10, bg: C.secBg, color: C.label });
            cells.push({ row: skillRow, col: 4, value: bonusFormula, fontSize: 10, align: 'center', bg: C.secBg, color: skill.proficient ? C.acGreen : C.label });
            skillRow++;
        }
    ...
    cells.push({ row: inspRow, col: 0, value: 'HEROIC', bold: true, fontSize: 9, bg: C.secHdr, color: C.secHdrTx, align: 'center' });
    cells.push({ row: inspRow + 1, col: 0, value: 'INSPIRATION', bold: true, fontSize: 9, bg: C.secHdr, color: C.secHdrTx, align: 'center' });
    ...
    cells.push({ row: 7, col: 10, value: 'armor of either type.', fontSize: 9, bg: C.acBg, italic: true, color: C.label });

    // Starting equipment card (armor)
    cells.push({ row: 9, col: 9, value: 'STARTING CARD', bold: true, fontSize: 9, bg: C.secHdr, color: C.secHdrTx, align: 'center' });
    cells.push({ row: 9, col: 10, value: '', bg: C.secHdr });
    cells.push({ row: 10, col: 9, value: 'Chain Shirt', locked: false, fontSize: 12, align: 'center', bg: C.secBg, color: C.editable });
    cells.push({ row: 10, col: 10, value: '', bg: C.secBg });

    // ─── EQUIPMENT (weapon area) ───
    cells.push({ row: 13, col: 6, value: 'You can use Simple', fontSize: 9, bg: C.secBg, italic: true, color: C.label });
    cells.push({ row: 13, col: 7, value: 'Weapons by placing', fontSize: 9, bg: C.secBg, italic: true, color: C.label });
    cells.push({ row: 14, col: 6, value: 'an Equipment Card for', fontSize: 9, bg: C.secBg, italic: true, color: C.label });
    cells.push({ row: 14, col: 7, value: 'a Simple weapon.', fontSize: 9, bg: C.secBg, italic: true, color: C.label });

    cells.push({ row: 16, col: 6, value: 'STARTING CARD', bold: true, fontSize: 9, bg: C.secHdr, color: C.secHdrTx, align: 'center' });
    cells.push({ row: 16, col: 7, value: '', bg: C.secHdr });
    cells.push({ row: 17, col: 6, value: 'Mace', locked: false, fontSize: 12, align: 'center', bg: C.secBg, color: C.editable });
    ...
    cells.push({ row: 14, col: 9, value: 'When you cast a spell', fontSize: 9, bg: C.secBg, italic: true, color: C.label });
    cells.push({ row: 14, col: 10, value: 'that requires', fontSize: 9, bg: C.secBg, italic: true, color: C.label });
    cells.push({ row: 15, col: 9, value: 'Concentration, place', fontSize: 9, bg: C.secBg, italic: true, color: C.label });
    ...
    // ─── ATTACK ROLLS ───
    const profRef = 'F5';
    cells.push({ row: 20, col: 6, value: 'MELEE ATTACK:', bold: true, fontSize: 10, bg: C.secBg, color: C.label });
    cells.push({ row: 20, col: 7, value: `=${modRefs['STR']}+${profRef}`, bold: true, fontSize: 16, align: 'center', bg: C.secBg, color: C.acRed });

    cells.push({ row: 21, col: 6, value: 'RANGED ATTACK:', bold: true, fontSize: 10, bg: C.secBg, color: C.label });
    cells.push({ row: 21, col: 7, value: `=${modRefs['DEX']}+${profRef}`, bold: true, fontSize: 16, align: 'center', bg: C.secBg, color: C.acRed });

    // ─── SPELL ATTACK & SAVE DC ───
    cells.push({ row: 20, col: 9, value: 'SPELL ATTACK:', bold: true, fontSize: 10, bg: C.secBg, color: C.label });
    cells.push({ row: 20, col: 10, value: `=${modRefs['WIS']}+${profRef}`, bold: true, fontSize: 16, align: 'center', bg: C.secBg, color: C.acRed });

    cells.push({ row: 21, col: 9, value: 'SPELL SAVE DC:', bold: true, fontSize: 10, bg: C.secBg, color: C.label });
    ...
    cells.push({ row: 6, col: 14, value: 'to cast a level 1 spell from', fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: 7, col: 13, value: 'one of your Spell Cards.', fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: 7, col: 14, value: 'Casting a Cantrip doesn\'t', fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: 8, col: 13, value: 'require a Power Token.', fontSize: 9, bg: C.secBg, color: C.label });

    cells.push({ row: 10, col: 13, value: 'Playing a Cleric', bold: true, fontSize: 12, bg: C.secBg, color: C.label });
    cells.push({ row: 11, col: 13, value: 'Choose this class if you', fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: 11, col: 14, value: 'want to play a holy warrior', fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: 12, col: 13, value: 'who invokes divine magic', fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: 12, col: 14, value: 'to heal, bolster, and smite.', fontSize: 9, bg: C.secBg, color: C.label });

    cells.push({ row: 14, col: 13, value: 'Cleric Tips', bold: true, italic: true, fontSize: 10, bg: C.secBg, color: C.label });
    cells.push({ row: 15, col: 13, value: 'In combat, defend your fellow', fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: 15, col: 14, value: 'party members with your', fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: 16, col: 13, value: 'trusty Mace and spells like', fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: 16, col: 14, value: 'Guiding Bolt.', italic: true, fontSize: 9, bg: C.secBg, color: C.label });

    cells.push({ row: 18, col: 13, value: 'Gaining a Level', bold: true, italic: true, fontSize: 10, bg: C.secBg, color: C.label });
    cells.push({ row: 19, col: 13, value: 'When you advance to level 2,', fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: 19, col: 14, value: 'flip this Class Board to the', fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: 20, col: 13, value: 'level 2 side and retrieve the', fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: 20, col: 14, value: 'additional components.', fontSize: 9, bg: C.secBg, color: C.label });

    // ─── RIGHT TABS ───
    cells.push({ row: 4, col: 16, value: 'SPECIES', bold: true, fontSize: 9, bg: C.acGold, color: '#ffffff', align: 'center' });
    cells.push({ row: 4, col: 17, value: 'CARD', bold: true, fontSize: 9, bg: C.acGold, color: '#ffffff', align: 'center' });
    cells.push({ row: 14, col: 16, value: 'BACKGROUND', bold: true, fontSize: 8, bg: C.acGold, color: '#ffffff', align: 'center' });
    cells.push({ row: 14, col: 17, value: 'CARD', bold: true, fontSize: 9, bg: C.acGold, color: '#ffffff', align: 'center' });
    ...
    cells.push({ row: needRow + 2, col: 1, value: 'Bless, Cure Wounds, Guiding Bolt, Light, Sacred Flame', locked: false, fontSize: 9, bg: C.secBg, color: C.editable });
    cells.push({ row: needRow + 3, col: 0, value: 'Starting Gold:', bold: true, fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: needRow + 3, col: 1, value: '60 GP', locked: false, fontSize: 9, bg: C.secBg, color: C.editable });

    // ─── REST SECTION ───
    cells.push({ row: needRow, col: 13, value: 'After a Short Rest:', bold: true, fontSize: 10, bg: C.secBg, color: C.label });
    cells.push({ row: needRow + 1, col: 13, value: '• Regain 4 Hit Point Tokens', fontSize: 9, bg: C.secBg, color: C.label });
    cells.push({ row: needRow + 2, col: 13, value: 'After a Long Rest:', bold: true, fontSize: 10, bg: C.secBg, color: C.label });
    cells.push({ row: needRow + 3, col: 13, value: '• Regain all HP & Power Tokens', fontSize: 9, bg: C.secBg, color: C.label });
